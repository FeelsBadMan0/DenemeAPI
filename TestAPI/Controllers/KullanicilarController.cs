...
namespace TestAPI.Controllers
{
    public class KullanicilarController : ApiController
    {
        ...
        public List<KULLANICILAR> KullaniciListele()
        {
            ...
            {
                string listele = "Select k.ID,k.KAYITKODU,k.KULLANICIADI,u.UYGULAMAADI AS 'UYGULAMA',r.ROL,k.KAYITTARIHI,k.AKTIF from KULLANICILAR k INNER JOIN UYGULAMALAR u on u.KAYITKODU=k.UYGULAMA INNER JOIN ROLLER r on r.KAYITKODU=k.ROL WHERE K.UYGULAMA='ef83de23-3d98-4db2-a558-402d3a2d8831' and k.AKTIF=1";
        ...
        public IHttpActionResult KullaniciGetir(KULLANICILAR kullanici)
        {
            ...
            if (UYGULAMA == "ef83de23-3d98-4db2-a558-402d3a2d8831")
            {
                if (!string.IsNullOrEmpty(kullanici.KAYITKODU))
                {
                    string kullaniciGetir = "Select k.ID,k.KAYITKODU,k.KULLANICIADI,u.UYGULAMAADI AS 'UYGULAMA',r.ROL,k.KAYITTARIHI,k.AKTIF from KULLANICILAR k INNER JOIN UYGULAMALAR u on u.KAYITKODU=k.UYGULAMA INNER JOIN ROLLER r on r.KAYITKODU=k.ROL WHERE K.UYGULAMA='ef83de23-3d98-4db2-a558-402d3a2d8831' and k.AKTIF=1 AND k.KAYITKODU=@KAYITKODU";
                    object parametreler = new
                    {
                        @KAYITKODU = kullanici.KAYITKODU
                    };
